...
loginApp.controller('loginController',function($scope,$http){
    ...
    $scope.postForm = function(){
        ...
        $http({
            method:"POST",
            url:'./get.php',
            /*$.param() 将对象{username:1,password:1}转成username=1&password=1*/
            /*$.formData*/
            data: $.param($scope.formData),
            /* post请求一定要加 headers:{'content-Type:'application/x-www-form-urlencoded'}*/
            headers:{'Content-Type':'application/x-www-form-urlencoded'}
        }).success(function(data){
            /*$http将返回的data(json格式)转成对象格式*/
            //console.dir(data);
            if(!data.success){
                //当前登录失败
                if(!data.errors){
...
pageList.controller('arcListCtrl',function($scope,$http,$location){
    ...
    $http({
        ...
        console.log('ni')
        $scope.ybyPaginationConf.totalItems=data.total;
    }).error(function(err){
        console.log(err);
    })
    //设置分页的对象
    ...
    //            /*
    //            * action:get_list
    //            * offset:$scope.limit 从第几条开始查询
    //            * rows:$scope.paginationConf.itemsPerPage 每页显示几条
    //            * orderField=id 根据其id值来排序
    //            * orderBy=DESC 降序
    //            * */
    ...
    //}
    //删除开始
    $scope.ybyPaginationConf={
        currentPage:1,
        itemsPerPage:2,
        onChange:function(){
            console.log('hao')
            if($scope.ybyPaginationConf.currentPage == 1){
                $scope.limit=0;
            }else {
                $scope.limit=($scope.ybyPaginationConf.currentPage-1)*
                $scope.ybyPaginationConf.itemsPerPage;
            }
            if($scope.typeid==0){
                $geturlyby = 'get.php?action=get_list&offset=' + $scope.limit + '&rows=' + $scope.ybyPaginationConf.itemsPerPage +
                '&orderField=id&orderBy=DESC';
            }else{
                $geturlyby='get.php?action=get_list&offset='+$scope.limit+
                '$rows='+$scope.ybyPaginationConf.itemsPerPage+'&where=typeid='+$scope.typeid
                +'&orderField=id&orderBy=DESC';
            }
            $http({
                method:'GET',
